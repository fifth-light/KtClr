...
class PeFileTest {
    ...
    fun testPeFile() {
        ...
        Unpooled.wrappedBuffer(bytes).use { buf ->
            ...
            createRvaMappedBuf(buf, sections).use { rvaBuf ->
                val cli = CliHeader(rvaBuf.slice(cliHeader.rva.toInt(), CliHeader.SIZE))
                ...
                val metadataRoot = MetadataRoot(rvaBuf.slice(cli.metaData.rva.toInt(), cli.metaData.size.toInt()))
