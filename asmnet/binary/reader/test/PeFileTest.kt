...
class PeFileTest {
    ...
    fun testReadRealPeFile() {
        ...
        val dosHeader = DosHeader(buf)
        assertEquals(DosHeader.MAGIC_MZ, dosHeader.e_magic)
        assertTrue(dosHeader.e_lfanew > 0u, "e_lfanew should be positive")

        val peOffset = dosHeader.e_lfanew.toInt()
        val coffHeader = CoffHeader(buf.slice(peOffset + 4, CoffHeader.SIZE))
        assertTrue(
        ...
        val optionalHeader = OptionalHeader(buf.slice(peOffset + 4 + CoffHeader.SIZE, buf.capacity() - peOffset - 4 - CoffHeader.SIZE))
        assertTrue(
